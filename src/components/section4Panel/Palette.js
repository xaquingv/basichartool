  ...
  render() {
    ...
    const defaultPaletteColors =
      colors.slice(0, -1).map((color, i) =>
        <li key={i} style={{backgroundColor: color}} onClick={this.onClick.bind(this, color)}></li>
      )

    // custom color
    const editableColorInput =
      <li className="li-custom" style={{borderColor: this.colorStyle}} onClick={this.onClick.bind(this, this.colorStyle)}>
    ...
    const pickedColor = colorPicked ? (
      <span className="p-r">(
        ...
        <span className="ml-10"> is picked)</span>
      </span>
